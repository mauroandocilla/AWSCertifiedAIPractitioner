...
| `/mapa-conceptual` | `ConceptMapPage` → `ConceptMap` | Grafo de cómo se conectan los términos entre sí (ver más abajo) |
| `/recorrido` | `StudyJourneyPage` | Checklist de avance por dominio/subsección/bullet (ver más abajo) |
| `/mapa-de-experiencia` | `JourneyMapPage` | Journey map de UX de preparar el examen con esta app (ver más abajo) |
...
Todas las rutas salvo `/` comparten una única `<Route element={<Layout />}>` (`src/components/Layout.tsx` = `QuickJumpBar` + `<Outlet/>`). `QuickJumpBar` deriva cuál pill resaltar a partir de la URL actual (no recibe `current` por prop) — esto es deliberado: antes cada página montaba su propio `QuickJumpBar`, así que React lo desmontaba y remontaba en cada navegación, perdiendo el scroll horizontal de la fila de pills. Con el layout compartido, `QuickJumpBar` se monta una sola vez para toda la sesión de navegación. `ThemeToggle`, `ScrollToTop`, `DomainSearch` (la búsqueda global) y `NowPlayingButton` (ver "Glosario hablado" más abajo) siguen el mismo patrón: montados una vez en `App.tsx`, fuera de `Routes`. Agregar una ruta nueva implica tocar cuatro lugares a la vez: el `<Route>`, la lista `ITEMS` de `QuickJumpBar.tsx`, `currentFromPath()` en `Layout.tsx`, y normalmente una tarjeta en la grilla de `Landing.tsx`.
...
Dos motores de reproducción intercambiables, con la misma API de hook (`status/activeIndex/rate/setRate/currentTime/duration/seek/seekTo/playAll/playRange/pause/resume/stop`), elegidos en tiempo real por `useAudioAvailable.ts`:
- `useReadAloud.ts` — `window.speechSynthesis` del navegador (`es-ES`), siempre disponible, sin configuración. `currentTime`/`duration` siempre son 0 y `seek`/`seekTo` no hacen nada — la Web Speech API no expone posición real dentro de una utterance.
- `useAudioReadAloud.ts` — reproduce MP3 pre-generados a través de un `<audio>` real desde `AUDIO_BASE` (`src/audioBase.ts`) + `${segmentId}.mp3`, y conecta `navigator.mediaSession` para controles de pantalla bloqueada — lo que permite que siga sonando con la pantalla bloqueada, algo que `speechSynthesis` no garantiza en iOS. `currentTime`/`duration` salen del propio `<audio>`; `seek(deltaSeconds)` pasa al segmento vecino (con el offset correcto) cuando un salto se pasa del principio/final del segmento actual, así un salto de ±5s cerca de un límite de segmento igual mueve ~5s reales de audio.
...
**El motor y su UI viven en `GlossaryAudioProvider.tsx`, montado una sola vez en `App.tsx` por encima de `<Routes>` — no adentro de `GlossaryEntryContent.tsx`.** Es deliberado, y arregla dos bugs que eran efecto secundario del enfoque anterior (uno por instancia): girar el teléfono cruza el mismo breakpoint de 720px que `DomainDetail.tsx` usa para cambiar toda su rama de layout, lo que antes desmontaba/remontaba lo que se estuviera escuchando como efecto colateral; y el reproductor flotante (`position: fixed`) terminaba pegado al fondo del *contenido* de un panel móvil en vez de al fondo de la pantalla, porque la animación CSS de ese panel deja un `transform: translateX(0)` permanente en sí mismo al terminar (`animation-fill-mode: both`) — y, según la spec, cualquier `transform` que no sea `none` en un ancestro, aunque sea un no-op, lo convierte en el bloque de contención de un descendiente `position: fixed`. Vivir afuera de cualquier layout de página esquiva ambos problemas de raíz. `GlossaryEntryContent.tsx` y `ResumeListeningBanner.tsx` solo consumen `useGlossaryAudio()`; una instancia dada solo trata el estado como "sobre mí" cuando `session.entryId === id`, porque lo que está sonando puede ser otra entrada (posiblemente en una página de la que ya te fuiste) — la reproducción es deliberadamente independiente de qué página está a la vista, al estilo podcast.

También vive en el provider:
- **Continuar escuchando**: la posición de reproducción más reciente en toda la app (entrada, segmento, segundo exacto) — no un tracker de completitud por entrada — guardada en `localStorage` (`glossary-audio-resume`) al pausar/detener/ocultar la pestaña/cerrar, y cada pocos segundos mientras suena. `ResumeListeningBanner.tsx` (en Inicio y `/glosario`) y la vista de pantalla completa de abajo leen `resumeState`/llaman a `resumeEntry()`.
- **Avance entre bullets**: "siguiente concepto" (`stepCard()`) cae al siguiente bullet en orden de lectura (`orderedGlossIds`, aplanado desde `domainData.ts` igual que `domainSearch.ts`) una vez que se acaban las tarjetas de la entrada actual, en vez de simplemente parar — una escucha continua de todo el glosario, no bullet por bullet. Simétrico hacia atrás (cae en la última tarjeta del bullet *anterior*, no en la primera).
- **`entryContentUrl`/`contentNavTarget`**: "ir al contenido" (en el mini-reproductor, el banner de resumen, y la vista de pantalla completa) espeja exactamente el `goToResult()` de `DomainSearch.tsx` — `highlightCardIndex` viaja como estado de navegación del router, que es lo que realmente hace que `GlossaryEntryContent.tsx` haga scroll y resalte el objetivo (un simple parámetro `?b=` en la URL no hace ninguna de las dos cosas). Ese objetivo también lleva un `nonce` (estado `highlight` de `DomainDetail.tsx`, no la URL) — sin él, navegar a una tarjeta que ya estabas mirando pasa el mismo `highlightCardIndex` que la vez anterior, y React se salta el efecto entero porque nada cambió en su arreglo de dependencias. También: los efectos de scroll de `GlossaryEntryContent.tsx` están deliberadamente demorados ~320ms (más allá de la animación de entrada de `.mobile-pane`) — llamar a `scrollIntoView` mientras esa animación sigue corriendo mide un objetivo que todavía se está moviendo, y termina una tarjeta o dos desviado una vez que asienta.
- **Now Playing**: `NowPlayingButton.tsx` es un ícono fijo en el header (presente en toda página, a diferencia del banner de resumen o el mini-reproductor) que abre un overlay de pantalla completa (estado `nowPlayingOpen`) mostrando lo que sea que esté sonando, o el aviso de reanudar si está inactivo, o un estado vacío.

El mini-reproductor flotante y la vista de pantalla completa comparten el JSX de la fila de progreso/controles (`renderProgressRow()`/`renderControlsRow()` dentro de `GlossaryAudioProvider.tsx`, reusado vía selectores CSS descendientes bajo `.now-playing-card` para la escala más grande, no duplicado).

## Íconos

`lucide-react` — todo ícono de la app es un import directo desde ahí (`import { Play, Pause, X, ... } from 'lucide-react'`) en el lugar donde se usa, sin componentes wrapper por ícono. Las dos excepciones son `SkipBackIcon.tsx`/`SkipForwardIcon.tsx`, que combinan una flecha de rotar de lucide con el número de segundos superpuesto como texto (no existe un ícono "saltar N segundos" en ninguna librería) — mantené esos dos, pero no agregues un wrapper nuevo para un ícono que es simplemente un re-export 1:1; es una indirección sin beneficio ahora que el ícono de base es un import de librería y no un SVG a mano.

## Mapa conceptual (`/mapa-conceptual`)

`ConceptMap.tsx` arma un grafo con fuerzas (`d3-force` + `d3-zoom`, renderizado SVG a mano, no una librería de canvas/WebGL) de términos *individuales* del glosario (no bullets completos) — `src/conceptGraph.ts` (`conceptEdges`) es una lista curada a mano de `{ from, to, relation, note? }`, donde `relation` es una de `requiere | ejemplo-de | se-usa-con | contrasta-con` y los ids de nodo son `${glossId}#${cardIndex}` (la posición de esa tarjeta dentro del html de esa entrada, el mismo esquema posicional que ya usa `domainSearch.ts`). No hay una lista de nodos separada — los nodos se derivan en tiempo real de qué ids aparecen en `conceptEdges`, buscando título/dominio en vivo vía `glossaryById` + `parseGlossaryCards()`, así nunca pueden desincronizarse del contenido real del glosario. Un chequeo de sanidad solo-dev al final de `conceptGraph.ts` (gateado por `import.meta.env.DEV`) avisa si algún edge tiene un id que no resuelve a una entrada/índice de tarjeta real — los ids curados a mano son justo el tipo de cosa que se tipea mal en silencio.

La simulación corre completamente afuera del ciclo de render de React: `nodes`/`links` se arman una sola vez (`useMemo`, nunca `useState`, porque `d3-force` los muta en el lugar en cada tick) y cada tick escribe directo a los atributos del DOM vía refs — hacer esto pasando por el estado de React 60 veces por segundo re-renderizaría todo el árbol en cada frame. Un burst síncrono de 300 ticks corre antes de mostrar el SVG, así el layout no aparece volando desde sus posiciones iniciales.

**Sobre la calidad de la curación**: la primera pasada de curación (elegir a mano conexiones reales leyendo las 310 tarjetas) produjo 51 grupos desconectados entre sí — sobre todo parejas/tríos sueltos que, bajo la sola repulsión de `forceManyBody` sin nada que los atraiga, se alejan arbitrariamente de la masa principal y se leen como "nada se conecta con nada" aunque dentro de su propio grupito sí lo hagan. Dos mitigaciones, ambas para tener en cuenta si se cura más contenido: (1) una segunda pasada de curación agregó conexiones puente entre los grupos chicos para reducirlos a un par de grupos grandes y cohesivos; (2) la simulación también aplica una fuerza `forceX`/`forceY` débil que atrae cada nodo hacia el centro sin importar su conectividad, así cualquier grupo que siga siendo chico se queda cerca del medio en vez de irse fuera del canvas. Si se agregan más edges curados después, conviene chequear la cantidad de componentes conectados resultante (un script descartable: sacar con regex cada par `from`/`to`, inundar un mapa de adyacencia) en vez de asumir que más edges siempre da un mapa más cohesivo.

## Recorrido de estudio (`/recorrido`)

`StudyJourneyPage.tsx` es un checklist manual sobre la estructura *ya existente* de `domainData.ts` (dominios → subsecciones → bullets) — sin archivo de contenido nuevo. No hay forma automática de saber si alguien realmente estudió un bullet (a diferencia del tracking de respuestas/aciertos del quiz o de la posición de resumen del audio), así que la completitud es autoreportada: tocar el checkbox de un bullet cambia un `Record<glossId, boolean>` persistido en `localStorage` (`study-journey-progress`). El progreso por dominio y total reusa `ProgressRing.tsx` (el mismo anillo que ya usa el sidebar de `QuizSessionPage.tsx` para el progreso por set).

## Mapa de experiencia (`/mapa-de-experiencia`)

`JourneyMapPage.tsx` es otra cosa distinta de las dos páginas de arriba — un journey map clásico de UX research (persona, acciones, comportamiento, sentimientos, una curva de ánimo, una fila de oportunidades), no un grafo de red ni un checklist. `STAGES` es un arreglo fijo escrito a mano (6 etapas de prepararse para el examen); la curva de ánimo es un gráfico de líneas SVG chico hecho a mano (una sola serie, un solo tono de acento — sin librería, esto no es un gráfico de exploración de datos). La fila de "oportunidades" es el sentido de adaptar este formato a una app en vez de un diagrama estático: cada etapa enlaza a la función real de esta app que ayuda en ese momento (ej. la etapa de práctica enlaza a `/quiz`, la etapa de simular el examen enlaza a `/examen`). Todas las filas se renderizan como una sola grilla CSS plana (`.jmap-grid`, `grid-template-columns: 110px repeat(6, ...)`) en vez de grillas anidadas por fila — una grilla anidada dejaría que cada fila calcule el ancho de sus propias columnas según solo el contenido de esa fila, así que la columna de una misma etapa podría terminar con un ancho distinto en la fila "Qué hace" que en la fila "Cómo se siente", rompiendo lo único que un journey map necesita (leer una columna de arriba a abajo como una sola etapa coherente).
